...
async def stream_chat_message(request: ChatMessageRequest) -> StreamingResponse:
    """Real-time SSE event stream for interactive chat and live UI control."""
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # 1. Stream UI Actions first so frontend updates visibly immediately
            ...
            # 2. Stream tokens of salesperson message
            ...
            # 3. Stream Done event
            yield f"event: done\ndata: {json.dumps({'intent': result['intent'], 'products_count': len(result.get('retrieved_products', []))})}\n\n"
